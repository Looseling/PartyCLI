...
public class NordVpnApiClient : IVpnServerApiClient
{
    ...
    public async Task<List<Server>> GetServerByProtocolListAsync(Protocols vpnProtocol,
        CancellationToken cancellationToken = default)
    {
        _logger.LogDebug("Fetching servers for protocol {Protocol}", vpnProtocol);

        var vpnProtocolId = (int)vpnProtocol;
        var url = $"servers?filters[servers_technologies][id]={vpnProtocolId}";
    ...
    public async Task<List<Server>> GetServerByCountryListAsync(int countryId,
        CancellationToken cancellationToken = default)
    {
        _logger.LogDebug("Fetching servers for country {CountryId}", countryId);

        var url = $"servers?filters[servers_technologies][id]={Protocols.NordLynx}&filters[country_id]={countryId}";
        var response = await _httpClient.GetAsync(url, cancellationToken);
